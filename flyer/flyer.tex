\documentclass[notumble,combine]{leaflet}
%\documentclass[a4paper,notumble,combine,debug]{leaflet}
%\usepackage[utf8]{inputenc}
%\usepackage[T1]{fontenc}
%\usepackage{lmodern}
%\renewcommand{\bfdefault}{b}
%\usepackage{microtype}
\usepackage[dvipsnames,usenames]{color}
%\usepackage[dvipdfmx]{graphicx}
%\usepackage[dvipdfmx]{graphicx, color}
\usepackage{graphicx}
\usepackage{alltt}
\usepackage[dvipdfmx]{hyperref}
\usepackage{ascmac}
\usepackage{comment}
\usepackage{here}
\usepackage{wrapfig}
\usepackage{floatflt}
%\usepackage{ifthen}

\makeatletter
% Section color
%% from leaflet.cls
\renewcommand\section{\@startsection{section}{1}{\z@}%
  {-3.5ex \@plus -.75ex}%
  {1ex} %{1.5ex}%
  {\normalfont\Large\sectfont\color{NavyBlue}\addul}}
\newcommand{\addul}[1]{\underline{#1}}
\renewcommand\subsection{\@startsection{subsection}{2}{\z@}%
  {-2.5ex plus -.5ex}%
  {1\p@} %{1ex}%
  {\normalfont\large\sectfont\color{Green}\addul}}
  \renewcommand\subsubsection{\@startsection{subsubsection}{2}{\z@}%
  {-1.5ex plus -.25ex}%
  {0.5\p@} %{1ex}%
  {\normalfont\normalsize\sectfont\color{Emerald}\addul}}
  \makeatother

\graphicspath{{figures/}} 

\title{
	\resizebox{\linewidth}{!}{\bf\color{Orange} ｽﾀｯｸﾁｬﾝ}\\
	\vfill
  \parbox[c]{5cm}{\includegraphics[width=5cm]{img/stackchan-photo.eps}}\\
%	[\baselineskip]
	\vfill
  \resizebox{\linewidth}{!}{\bf \color{Red} コミュニケーションロボットをあなたの手に。}\\
  \resizebox{\linewidth}{!}{\bf \color{Red} ｽﾀｯｸﾁｬﾝはなにをしなくても、スーパーかわいい}\\
        \vfill
  \includegraphics[width=2cm]{img/Stackchan_flyer.eps}\\
  {\bf\normalsize 電子チラシ(リンクはこちら)}\\
  {\bf \href{https://scrapbox.io/stack-chan/}{ｽﾀｯｸﾁｬﾝコミュニティ}}\\
}



\date{2025年3月15日(土)大阪もくもく会版}
%\CutLine*{1} \CutLine*{2} \CutLine*{3} \CutLine*{4} \CutLine*{5}
%\CutLine*{6}

% \AddToBackground*{2}{% Background of a large page
%   \put(\LenToUnit{.25\paperwidth},\LenToUnit{.4\paperheight}){%
%     \includegraphics[width=14.85cm]{kbug-logo}
% }}

\hypersetup{
setpagesize=false,
 bookmarksnumbered=true,
 bookmarksopen=true,
 colorlinks=true,
 linkcolor=blue,
 citecolor=red,
}

\begin{document}
\maketitle
\thispagestyle{empty}
\pagebreak{}
\section{ｽﾀｯｸﾁｬﾝってなぁに?}
ｽﾀｯｸﾁｬﾝはししかわさんが開発、公開している、 手乗りサイズのｽｰﾊﾟｰｶﾜｲｲコミュニケーションロボットです。 
作品ページ：\url{https://github.com/meganetaaan/stack-chan}
\begin{itemize}
  \item \resizebox{\linewidth}{!}{\bf\color{Red}コミュニケーションロボットをあなたの手に。}
  \item \resizebox{\linewidth}{!}{\bf\color{Red}ｽﾀｯｸﾁｬﾝはなにをしなくても、スーパーかわいい}
\end{itemize}

\includegraphics[width=\textwidth]{img/stackchan-salespoint.eps}

ｽﾀｯｸﾁｬﾝの魅力は、以下のとおりです。
\begin{itemize}
  \item そこにいるだけでうれしい
  \item ランダムに動いてかわいい
  \item お出かけにピッタリのサイズ
  \item 自分でプログラムできる
  \item コミュニティが暖かい
\end{itemize}
以下のようなものになれます。
\begin{itemize}
    \item Bluetoohスピーカになる
    \item AIになる
    \item radikoプレーヤーになる
    \item MP3プレーヤーになる
    \item 音に合わせて口パクしてくれる
%    \item 自分の好きなお顔にできる
\end{itemize}

\section{ｽﾀｯｸﾁｬﾝのはじめかた}

\subsection{お顔だけでも楽しいですよ!!}
ｽﾀｯｸﾁｬﾝの多くのプログラムは、からだがなくてもM5Stack単体だけで動作します。
たくさんのプログラムが、M5Stack用のプログラム書き込みツールM5Burnerで提供されています。


\subsection{あなたはどのｽﾀｯｸﾁｬﾝ?}
ｽﾀｯｸﾁｬﾝを作成するには、いくつかの選択肢があります。
例えば、M5Stackになにを選ぶのか、からだ(筐体)をどうするか、サーボモータにどんなものを使うのかなどです。

動くｽﾀｯｸﾁｬﾝを作るためには、以下のものが必要です。

\begin{itemize}
  \item M5Stack本体
  \item 3Dプリンタなどで作ったからだ(筐体)
  \item サーボモータと専用基板
\end{itemize}

\subsubsection{M5Stackを選ぶ}
M5Stackは、これらを選ぶと良いと思います。
\begin{itemize}
  \item \href{https://www.switch-science.com/products/6530}{Core2} v1.0 or 
        \href{https://www.switch-science.com/products/6784}{Core2 aws}: 色々試したければこれ。
  \item \href{https://www.switch-science.com/products/8960}{CoreS3}: カメラもあって夢が広がる。
  \item \href{https://www.switch-science.com/products/9010}{Basic(Core)}: 安価で手に入りやすい。
  \item \href{https://www.switch-science.com/products/6347}{Atom Echo}: AIｽﾀｯｸﾁｬﾝミニマルならこれ。
\end{itemize}

\subsubsection{からだ(筐体)を選ぶ}
\includegraphics[width=\textwidth]{img/body.eps}
からだには、たとえば以下のようなものがあります。
\begin{itemize}
  \item \href{https://raspberrypi.mongonta.com/about-products-stackchan-m5gobottom-version/}{タカオ版}:定番です。\href{https://mongonta.booth.pm/}{販売}されており手に入りやすいです。
  \item \href{https://booth.pm/ja/items/4796165}{マグネット版}:磁石でCore2を固定します。取り外しが簡単です。
  \item \href{https://github.com/syumme/stack-chan_cardbord_patternV3}{ダンボール版}:ダンボールでもからだを作れます。
  \item \href{https://github.com/A-Uta/AI_StackChan_Minimal}{AIｽﾀｯｸﾁｬﾝミニマル版}
  \item その他にも、\href{https://lang-ship.com/blog/work/pura-stack-chan/}{プラダン版}、\href{https://github.com/SuspiciousCEO/Mass-production-type-MDF-stack-chan}{MDF版}など
\end{itemize}

\subsubsection{サーボモーターに何を使うか}
簡単に利用できるサーボモーターには、以下のようなものがあります。
\begin{itemize}
  \item SG90: 安価ですが、寿命などの点で難があります\\
    対応ボード:\href{https://www.switch-science.com/products/8905}{Stack-chan\_Takao\_Base}
  \item シリアルサーボ(SCS0009): 高価ですが、寿命は長いです\\
    対応ボード:\href{https://www.switch-science.com/products/9288}{Stack-chan\_TakaoBase\_SCS0009}
\end{itemize}

\section{ｽﾀｯｸﾁｬﾝ3つのオープン}
ｽﾀｯｸﾁｬﾝは、以下の3つがオープンな環境です。

\subsection{オープンな仕様}
ｽﾀｯｸﾁｬﾝは、ソフトウエアがオープンになっていて、ソースが公開されています。

ハードウエアもオープンになっていて、からだ(筐体)データ(3Dプリントなど)や基板データがオープンです。

\subsection{オープンなプロセス}
ｽﾀｯｸﾁｬﾝは、 制作の過程もオープンになっています。
制作の過程がオープンとは、みんなが自分のｽﾀｯｸﾁｬﾝを作成するときの情報が共有されているということです。

ここでは、成功例が手に入るだけでなく、失敗例も手に入るようになっています。
ｽﾀｯｸﾁｬﾝのオープンなプロセスでは、難しいことを難しいと言える雰囲気を大事にしています。

\subsection{オープンなコミュニティ}
ｽﾀｯｸﾁｬﾝコミュニティは誰にでもひらかれています。
ｽﾀｯｸﾁｬﾝコミュニティでは、全員がユーザーであり開発者でもあります。

うちの子自慢も盛んに行われていますので、あなたのｽﾀｯｸﾁｬﾝも自慢してみてください。

最近は、もくもく会やオンリーイベントなども行われています。
詳しくは、イベント情報をご覧ください。

\section{色々なｽﾀｯｸﾁｬﾝ}
\includegraphics[width=\textwidth]{img/stackchans.eps}

ｽﾀｯｸﾁｬﾝには色々な種類のものがあります。
\begin{itemize}
  \item \href{https://github.com/mongonta0716/stackchan-bluetooth-simple}{Bluetoothスピーカー版}:リップシンクしてくれます
  \item \href{https://github.com/K-Yama2010/AI-Stack-Chan2-Knightrider-season2}{AI版}:音声入出力でAIエージェントとして動きます
  \item \href{https://github.com/robo8080/M5Unified_StackChan_Radiko}{Radiko版}:Radikoが聴けます
  \item 時計版:時計にもなります
  \item \href{https://github.com/akita11/Stack-chan_bit}{Stack-chan\_bit版}:microbitとも使えます
  \item \href{https://github.com/mongonta0716/stack-chan-tester}{動作テスター版}:動作確認/初期設定用です
\end{itemize}

\subsection{Bluetoothスピーカー版ｽﾀｯｸﾁｬﾝ}
ｽﾀｯｸﾁｬﾝがBluetoohスピーカーとして動作し、音に合わせてリップシンクしてくれます。
まるで、ｽﾀｯｸﾁｬﾝがしゃべっているようです!!

\subsection{AI版ｽﾀｯｸﾁｬﾝ}
AI版では、ｽﾀｯｸﾁｬﾝがAIになります!!
ｽﾀｯｸﾁｬﾝが音声で質問を聞いてくれ、ChatGPTやローカルLLMなどのAIに問い合わせ、答えを音声で教えてくれます。

\subsubsection{顔検出版CoreS3用AIｽﾀｯｸﾁｬﾝ}
CoreS3にはカメラがあり、これで
\href{https://github.com/ronron-gh/AI_StackChan2_DevCam}{顔検出してAI動作}
をしてくれます。

\subsubsection{AIｽﾀｯｸﾁｬﾝミニマル}
\href{https://github.com/A-Uta/AI_StackChan_Minimal}{AIｽﾀｯｸﾁｬﾝミニマル}は、誰にでも気軽にｽﾀｯｸﾁｬﾝが楽しめるようにというコンセプトで作られたバージョンです。
5000円程度で作ることができます。

\newpage
\section{関連イベント(予定)}
以下のようなイベントが予定されています。
\begin{itemize}
  \item 2025/03/15: \href{https://610t.connpass.com/event/345442/}{第1回大阪もくもく会}
    @\href{https://www.r3it.com/ashibinaa}{gusuku Ashibinna Osaka}(グランフロント大阪)
  \item 2025/03/23: \href{https://wiki.nicotech.jp/nico_tech/?NT%E4%BA%AC%E9%83%BD2025}{NT京都}
    @西院春日神社(京都)
  \item 2025/03/29: 第6回東京もくもく会@赤坂
  \item 2025/04/05: ｽﾀｯｸﾁｬﾝﾌｪｽ
    @\href{https://extinct-media-museum.blog.jp/otemachi/}{絶滅メディア博物館}(東京)
  \item 未定(7/2前後?): ｽﾀｯｸﾁｬﾝ4歳のお誕生日会2025
\end{itemize}
\vfill
\includegraphics[width=\textwidth]{img/stackchan-many.eps}
\vfill
\begin{minipage}{\textwidth}
\begin{boxnote}

\section{ｽﾀｯｸﾁｬﾝ情報}
ｽﾀｯｸﾁｬﾝに関する情報は、以下のような場所で入手することができます。

\begin{itemize}
  \item \href{https://scrapbox.io/stack-chan/}{Cosense:情報集約用}\\
    \href{https://scrapbox.io/stack-chan/}{https://scrapbox.io/stack-chan/}
  \item \href{https://discord.gg/eGhd9adnBm}{Dicsord}:\href{https://discord.gg/eGhd9adnBm}{https://discord.gg/eGhd9adnBm}
  \item X:
	\href{https://x.com/stack_chan}{@stack\_chan}
	ハッシュタグ:\href{https://x.com/search?q=%23%EF%BD%BD%EF%BE%80%EF%BD%AF%EF%BD%B8%EF%BE%81%EF%BD%AC%EF%BE%9D&src=typed_query&f=live}{\#ｽﾀｯｸﾁｬﾝ}
  \item mixi2:ｽﾀｯｸﾁｬﾝコミュニティ
  \item ProtoPedia:プロトタイプ投稿サイト\url{https://protopedia.net/material/833}
\end{itemize}

\includegraphics[width=2.2cm]{img/cosense.eps}
\includegraphics[width=2.2cm]{img/discord.eps}
\includegraphics[width=2.2cm]{img/X.eps}

\end{boxnote}
\end{minipage}
\end{document}
